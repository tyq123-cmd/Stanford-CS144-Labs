...
void TCPSender::fill_window() {
    do {
        size_t len_to_read = min(min(TCPConfig::MAX_PAYLOAD_SIZE, _stream.buffer_size()),
                                    static_cast<size_t>(_window_size));
        if (!len_to_read && _syn_num == 2) return;
        // if (!len_to_read && _window_size && _next_seqno) return;
        ...
        cout << _window_size << " " << _stream.input_ended() << " " << tcp_segment.length_in_sequence_space() << endl;
        if (!tcp_segment.length_in_sequence_space() && !_ack_received) return;
...
bool TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    if (window_size == 0) _ack_received = true;
    _consecutive_retrans = 0;
...
void TCPSender::retrainsmit(uint64_t abs_seqno) {
    _segments_out.push(_unacked_segments[abs_seqno]);
}

void TCPSender::send_empty_segment() {
    TCPSegment tcp_segment;
    // tcp_segment.parse(Buffer());
    auto& header = tcp_segment.header();
    header.seqno = wrap(_next_seqno, _isn);
    _segments_out.push(tcp_segment);
}
